...
int64_t
benchmark_similarity_transform(sycl::queue& q,
                               const uint dim,
                               const uint wg_size,
                               uint* const itr_count)
{
  ...
  int64_t tm =
    similarity_transform(q, mat, eigen_val, eigen_vec, dim, wg_size, itr_count);
...
int64_t
benchmark_sum_across_rows_kernel_v0(sycl::queue& q,
                                    const uint dim,
                                    const uint wg_size)
{
  float* mat = (float*)malloc(sizeof(float) * dim * dim);
...
int64_t
benchmark_sum_across_rows_kernel_v1(sycl::queue& q,
                                    const uint dim,
                                    const uint wg_size)
{
  float* mat = (float*)malloc(sizeof(float) * dim * dim);
...
int64_t
benchmark_sum_across_rows_kernel_v2(sycl::queue& q,
                                    const uint dim,
                                    const uint wg_size)
{
  float* mat = (float*)malloc(sizeof(float) * dim * dim);
...
int64_t
benchmark_find_vector_max_v0(sycl::queue& q, const uint dim, const uint wg_size)
{
  float* vec = (float*)malloc(sizeof(float) * dim * 1);
...
int64_t
benchmark_find_vector_max_v1(sycl::queue& q, const uint dim, const uint wg_size)
{
  float* vec = (float*)malloc(sizeof(float) * dim * 1);
  float* max = (float*)malloc(sizeof(float) * 1);
  int64_t tm = 0;
...
int64_t
benchmark_find_vector_max_v2(sycl::queue& q, const uint dim, const uint wg_size)
{
  float* vec = (float*)malloc(sizeof(float) * dim * 1);
  float* max = (float*)malloc(sizeof(float) * 1);
  int64_t tm = 0;
...
int64_t
benchmark_compute_eigen_vector_v0(sycl::queue& q,
                                  const uint dim,
                                  const uint wg_size)
{
  float* vec = (float*)malloc(sizeof(float) * dim * 1);
...
int64_t
benchmark_compute_eigen_vector_v1(sycl::queue& q,
                                  const uint dim,
                                  const uint wg_size)
{
  float* vec = (float*)malloc(sizeof(float) * dim * 1);
...
int64_t
benchmark_compute_next_matrix(sycl::queue& q,
                              const uint dim,
                              const uint wg_size)
{
  float* mat = (float*)malloc(sizeof(float) * dim * dim);
...
int64_t
benchmark_stop_criteria_tester(sycl::queue& q,
                               const uint dim,
                               const uint wg_size)
{
  float* vec = (float*)malloc(sizeof(float) * dim * 1);
